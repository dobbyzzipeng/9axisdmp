...
#define DEFAULT_SAMPLE_HZ  (500)//sampling rate@fsr 1KHz
...
short _gyro[3], _accel[3], _sensors;
unsigned char _more;
long quat[4];
...
#define i2c_write   MPU6500_Write_MultiBytes//i2cwrite
...
void mpu_dmp_init(void)
{
    ...
    if(!result)   //返回0代表初始化成功
    {
        ...
        //mpu_set_sample_rate
        if(!mpu_set_sample_rate(DEFAULT_SAMPLE_HZ))
        {
...
void dmp_get_angle(float *pit,float *rol,float *yaw)
{
	//float Yaw,Roll,Pitch;
    dmp_read_fifo(_gyro, _accel, quat, &sensor_timestamp, &_sensors, &_more);	 
     ...
     */
      ...
      */
      ...
      if (_sensors & INV_WXYZ_QUAT )
      {
         ...
         *pit  = asin(-2 * q1 * q3 + 2 * q0* q2)* 57.3f; // pitch
         *rol  = atan2(2 * q2 * q3 + 2 * q0 * q1, -2 * q1 * q1 - 2 * q2* q2 + 1)* 57.3f; // roll
         *yaw  = atan2(2*(q1*q2 + q0*q3),q0*q0+q1*q1-q2*q2-q3*q3) * 57.3f;
      }

      if(_sensors & INV_XYZ_GYRO)
      {}
      if(_sensors & INV_XYZ_ACCEL)
      {}
}
/**
 *  @}
